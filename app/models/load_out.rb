class LoadOut < ApplicationRecord
  ...
  pg_search_scope :global_search, against: [:reference, :status, :truck_nr, :trailer_nr, :departure_date, :type_of_service,:trip_ref], associated_against: {
      out_assignments: [:reference, :lot_nr, :incoming_order_ref,:incoming_transport_ref, :other_ref, :number_of_boxe, :number_of_pallet, :packer],
      t1_customer: [:name]
     },
      using: {
        tsearch: { prefix: true }
